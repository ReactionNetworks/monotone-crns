...
#	if dual_cone.dim() == dimension and len( dual_cone_rays_temp ) == dual_number_of_generators:
		cone = dual_cone.dual()
	...
	if len( preserved_crn_vectors ) > 1 and noninterconversion_vector_found and all( reactant_non_outflow ):
		...
		#print( 'with lexicographic ordering = ' + str( lexicographic_ordering ) )
...
if show_interconversions:
	...
	print( 'Found ' + str( number_of_interesting_cones ) + ' cones that preserve two or more reactions, including at least one non-interconversion reaction, out of a total of ' + str( len( dual_base_vectors ) ) + ' possible cones.' )
print( '' )
print( '' )
